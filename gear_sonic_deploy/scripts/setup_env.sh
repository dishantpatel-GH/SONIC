...
for install_path in "${ROS2_INSTALL_PATHS[@]}"; do
    ...
    for distro in "${ROS2_DISTROS[@]}"; do
        ...
        if [ -f "$ros2_setup_file" ]; then
            ...
            # CRITICAL: Prepend Unitree DDS library path to prevent ROS2's CycloneDDS (0.10.5 with
            # Iceoryx) from shadowing Unitree SDK2's CycloneDDS (0.10.2 __noshm). Without this,
            # libddscxx.so.0 loads ROS2's libddsc which has Iceoryx/m_iox_pub support that triggers
            # an assertion failure: "(wr->m_iox_pub == NULL) == (d->a.iox_chunk == NULL)"
            _UNITREE_DDS_LIB="$(pwd)/thirdparty/unitree_sdk2/thirdparty/lib/${ARCH}"
            if [ -d "$_UNITREE_DDS_LIB" ]; then
                export LD_LIBRARY_PATH="$_UNITREE_DDS_LIB:$LD_LIBRARY_PATH"
                echo "✅ Unitree DDS library path prioritized over ROS2 CycloneDDS to prevent version conflict"
            fi
            export HAS_ROS2=1
