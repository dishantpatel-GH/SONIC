...
if [[ -n "$NO_HANDS_ARG" ]]; then
echo -e "  Hands:              ${YELLOW}DISABLED (--no-hands)${NC}"
fi
...
if [[ "$confirm" =~ ^[Yy]$ ]] || [[ -z "$confirm" ]]; then
    ...
    if [[ -n "$EXTRA_ARGS" ]] || [[ -n "$NO_HANDS_ARG" ]]; then
